...
def banner (txt):
    ...
    print(bold + fgreen + """
{1}__        ______       _   _            _{5}
\ \      / /  _ \     / \ | |_ ___  ___| | __ {0} by {4}@Ero-hack{5}
 \ \ /\ / /| |_) |   / _ \| __/ _ \/ __| |/ /{5}  {2} Version {3}{1} K-RONA
  \ V  V / |  __/   / ___ \ ||  __/ (__|   <{5}
   \_/\_/  |_|     /_/   \_\__\___|\___|_|\_\{5}
""".format(orange, lblue, yellow, cmseek_version, red, white))
    if txt != "":
        print(whitebg + black + bold)
        print(" [+]  " + txt + "  [+] " + cln)
    else:
        print(cln + bold + lbluebg + black + " Author: " + cln + bold + "" + blackbg + white + "\n GitHub: " + cln + bold + " https://github.com/ERO-HACK/WP-Ateck \n" + cln + '\n')
    print(cln)
    return
...
def update_brute_cache():
    ...
    if not os.path.isdir(brute_dir):
        try:
            error('WP-Ateck could not find the bruteforce directory, Creating Brute directory')
            os.makedirs(brute_dir)
            info('Bruteforce directory created, add some modules from: https://github.com/ERO-HACK/WP-Ateck')
            bye()
    ...
    if not modules == [] and modulen != []:
        ...
        print('\n')
        result('Cache Updated! Enjoy WP-Ateck with new modules ;)','')
    else:
...
def update():
    ...
    if get_version[0] != '1':
        ...
        info("WP-Ateck Version: " + cmseek_version)
        ...
        if my_version > serv_version:
            ...
            print('\n')
            result("WP-Ateck is up to date, Thanks for checking update tho.. It's a good practise",'')
        else:
            print('\n')
